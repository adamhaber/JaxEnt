...
def test_indep_train_exhuastive_from_marginals(lr=1e-1):
    ...
    assert onp.array(m.model_marg)==pytest.approx(onp.array(indep_marg),abs=1e-2)

def test_indep_train_exhuastive_from_data():
    m = Indep(N_indep)
    m.factors = indep_factors
    data = m.sample(random.PRNGKey(0),10000,m.factors)
    
    m2 = Indep(N_indep)
    m2.train(data,threshold=1)
    assert onp.array(m2.model_marg)==pytest.approx(onp.array(indep_marg),abs=1e-2)
...
def test_ising_train_exhuastive_from_marginals():
    ...
    onp.array(m.model_marg),onp.array(ising_marg)
    assert onp.array(m.model_marg)==pytest.approx(onp.array(ising_marg),abs=1e-2)
...
def test_ising_train_exhuastive_from_data():
    ...
    m2 = Ising(N_ising)
    m2.train(data,threshold=1)
    assert onp.array(m2.model_marg)==pytest.approx(onp.array(ising_marg),abs=1e-2)